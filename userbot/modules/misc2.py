...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await event.edit("`Reply Ke Media`")
...
async def _(event):
    ...
    if not event.reply_to_msg_id:
        await event.edit("```Reply Ke Link.```")
        return
    reply_message = await event.get_reply_message()
...
async def _(event):
    ...
    await event.delete()
    await bot.send_message(
        event.chat_id, "`✓Clone Berhasil Dilakukan!`", reply_to=reply_message
    )
...
async def _(event):
    ...
    await event.edit("`✓ Berhasil Mengembalikan Akun!`")
...
CMD_HELP.update(
    {
        "randomprofil": "**Modules:** __Random Profil__\n\n**Perintah:** `.randompp`"
