...
async def upload_google_photos(event):
    ...
    async with aiohttp.ClientSession() as session:
        headers = {
            "Content-Length": "0",
            "ezbw-Goog-Upload-Command": "start",
            "ezbw-Goog-Upload-Content-Type": mime_type,
            "ezbw-Goog-Upload-File-Name": file_name,
            "ezbw-Goog-Upload-Protocol": "resumable",
            "ezbw-Goog-Upload-Raw-Size": str(file_size),
        ...
        real_upload_url = step_one_resp_headers.get("ezbw-Goog-Upload-URL")
        logger.info(real_upload_url)
        upload_granularity = int(
            step_one_resp_headers.get("ezbw-Goog-Upload-Chunk-Granularity")
        )
        ...
        async with aiofiles.open(file_path, mode="rb") as f_d:
            for i in range(number_of_req_s):
                ...
                headers = {
                    ...
                    "ezbw-Goog-Upload-Command": "upload",
                    "ezbw-Goog-Upload-Offset": str(offset),
                    "Authorization": "Bearer " + creds.access_token,
                }
                logger.info(i)
                logger.info(headers)
            ...
            headers = {
                ...
                "ezbw-Goog-Upload-Command": "upload, finalize",
                "ezbw-Goog-Upload-Offset": str(number_of_req_s * upload_granularity),
                "Authorization": "Bearer " + creds.access_token,
            }
            logger.info(headers)
            response = await session.post(
