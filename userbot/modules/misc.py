...
async def killdabot(event):
    await event.edit("`Restarting 𝙚𝙯𝙗𝙬...`")
    await asyncio.sleep(10)
    await event.delete()
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#RESTARTBOT \n" "`𝙚𝙯𝙗𝙬 Telah Di Restart`"
        )
    await bot.disconnect()
...
async def reedme(e):
    await e.edit(
        ...
        "\n[ezbw Repo](https://github.com/ez69bw/ezbw/blob/ezbw/README.md)"
...
async def repo_is_here(wannasee):
    ...
    await wannasee.edit("[Lord-Userbot](https://github.com/Zora24/Lord-Userbot)")
...
async def okgoogle(img):
    ...
    if message and message.media:
        ...
        await img.edit("`Reply Ke Gambar`")
        return
